...
Одна база (`class=base`), центры (`class=center`), курьерские точки (`class=roverSpawn`) и одна ремонтная (`class=repairSpawn`). ID совпадают с level meta. Для доменной классификации использовать современное поле Tiled `class`, не legacy `type`.
